...
#ifdef NS_ENABLE_SSL
    if (use_ssl) {
      nc->ssl_ctx = SSL_CTX_new(TLSv1_2_server_method());
      SSL_CTX_set_options(nc->ssl_ctx, SSL_OP_CIPHER_SERVER_PREFERENCE | SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1);
...
#ifdef NS_ENABLE_SSL
  if (use_ssl) {
    if ((nc->ssl_ctx = SSL_CTX_new(TLSv1_2_client_method())) == NULL ||
        SSL_CTX_set_options(nc->ssl_ctx, SSL_OP_CIPHER_SERVER_PREFERENCE | SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1) != 0 ||
...
int mg_terminate_ssl(struct mg_connection *c, const char *cert) {
  ...
  DBG(("%p MITM", conn));
  if ((ctx = SSL_CTX_new(TLSv1_2_server_method())) == NULL) return 0;

  SSL_CTX_set_options(ctx, SSL_OP_CIPHER_SERVER_PREFERENCE | SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1);
